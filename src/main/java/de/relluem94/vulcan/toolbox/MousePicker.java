...
public class MousePicker {
    ...
    private Vector3f getPointOnRay(Vector3f ray, float distance) {
        Vector3f camPos = Main.camc.getPosition();
    ...
    private Vector3f binarySearch(int count, float start, float finish, Vector3f ray) {
        ...
        if (count >= RECURSION_COUNT) {
            ...
            int id = Utils.getTerrain(new Vector2f(endPoint.getX(), endPoint.getZ()));
            if (terrain != null) {
                return endPoint;
            } else {
                return null;
    ...
    private boolean isUnderGround(Vector3f testPoint) {
        ...
        int id = Utils.getTerrain(new Vector2f(testPoint.getX(), testPoint.getZ()));
        float height = 0;
        if (terrain != null) {
            height = TerrainComponent.getHeightOfTerrain(id, testPoint.getX(), testPoint.getZ());
        }
